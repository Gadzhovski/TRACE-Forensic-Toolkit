...
#             self.operationCompleted.emit(False, f"Failed to mount the image. Error: {e.output.decode()}")
#
...
#             subprocess.run(['sudo', 'mount', '-o', 'loop,ro', os.path.join(ewf_mount_dir, 'ewf1'), '/mnt/disk_image'],
#                            check=True)
#         else:
...
#
#     def _dismount_image(self):
#         """Dismount the image based on the operating system."""
#         try:
#             if os.name == 'posix':  # macOS
#                 # Step 1: Get the disk identifier and corresponding mount points
#                 info_output = subprocess.check_output(['hdiutil', 'info'], stderr=subprocess.STDOUT).decode()
#                 mounted_disks = []
#                 for line in info_output.splitlines():
#                     if 'image-path' in line and self.image_path in line:
#                         current_disk = line.split()[0]
#                     if line.startswith('/dev/'):
#                         mounted_disks.append(line.split()[0])
#
#                 # Step 2: Detach each mounted disk associated with the image
...
#                 # Additionally, unmount the ewf directory if it was an E01 image
...
#
#         except subprocess.CalledProcessError as e:
#             raise Exception(f"Failed to dismount the image. Error: {e.output.decode()}")
#
#     def mount_image(self):
#         """Attempt to mount an image after prompting the user to select one."""
#         supported_formats = ("EWF Files (*.E01);;Raw Files (*.dd);;AFF4 Files (*.aff4);;"
...
# import platform
# from PySide6.QtCore import QThread, Signal
# from PySide6.QtWidgets import QFileDialog, QMessageBox, QWidget, QInputDialog
#
#
#
#
...
#             subprocess.run(['sudo', 'umount', '/mnt/disk_image'], check=True)
#
#             # Check if the image was an E01 and unmount ewf if necessary
#             if self.image_path.lower().endswith('.e01'):
#                 subprocess.run(['sudo', 'umount', '/mnt/ewf'], check=True)
#
#             self.operationCompleted.emit(True, "Image was dismounted successfully.")
...
#             image_path, _ = QFileDialog.getOpenFileName(QWidget(None), "Select Disk Image", "", supported_formats)
#
#             if not image_path:# #Description: This file contains the ImageManager class, which is a subclass of QThread. This class is responsible for mounting and dismounting disk images using Arsenal Image Mounter. It also emits signals to indicate the completion of an operation and to show messages to the user.
#
# import os
# import subprocess
# from PySide6.QtCore import QThread, Signal
